...
import { StudyMaterialType, GeneratedContent, History, HistoryItem } from './types';
...
const translations: Record<string, Record<string, string>> = {
  en: {
    ...
    chatbotTitle: "Chat with AI Tutor",
    chatbotWelcome: "Ask me anything about the selected chapter!",
  ...
  ms: {
    ...
    chatbotTitle: "Berbual dengan Tutor AI",
    chatbotWelcome: "Tanya saya apa sahaja tentang bab yang dipilih!",
    chatbotPlaceholder: "Taip soalan anda...",
    chatbotSend: "Hantar",
    generationHistory: "Sejarah Penjanaan",
  ...
  ar: {
    ...
    chatbotTitle: "دردش مع مدرس الذكاء الاصطناعي",
    chatbotWelcome: "اسألني أي شيء عن الفصل المحدد!",
...
const ProgressTracker: React.FC<{
    status: Partial<Record<StudyMaterialType, boolean>>;
    t: (key: string) => string;
    language: string;
}> = ({ status, t, language }) => {
    const materialTypes = [
        { type: StudyMaterialType.SUMMARY, icon: <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${language === 'ar' ? 'ml-1.5' : 'mr-1.5'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> },
        { type: StudyMaterialType.QUIZ, icon: <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${language === 'ar' ? 'ml-1.5' : 'mr-1.5'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
        { type: StudyMaterialType.TIMELINE, icon: <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${language === 'ar' ? 'ml-1.5' : 'mr-1.5'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
        { type: StudyMaterialType.FLASHCARDS, icon: <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${language === 'ar' ? 'ml-1.5' : 'mr-1.5'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg> },
    ];
    ...
    return (
        <div className="space-y-2 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
                {materialTypes.map(({ type, icon }) => {
                    const isCompleted = status[type];
                    return (
                        <div
                            key={type}
                            className={`flex items-center p-2 rounded-md text-xs font-medium transition-colors duration-300 ${
                                isCompleted
...
const App: React.FC = () => {
  ...
  const [completionStatus, setCompletionStatus] = useState<Record<string, Partial<Record<StudyMaterialType, boolean>>>>({});
  ...
  const handleSaveToHistory = (content: GeneratedContent, key: string, type: StudyMaterialType) => {
      const newHistoryItem: HistoryItem = {
          content,
          timestamp: new Date().toISOString(),
      };
      setHistory(prev => {
          const currentHistory = prev[key]?.[type] || [];
          const updatedHistory = [newHistoryItem, ...currentHistory].slice(0, 10); // Keep last 10
          return {
              ...prev,
              [key]: {
                  ...prev[key],
        ...
        const generateFromUrl = async () => {
            ...
            try {
                ...
                setCompletionStatus(prev => ({
                    ...prev,
                    [topicKeyFromUrl]: { ...prev[topicKeyFromUrl], [typeFromUrl]: true }
                }));
                handleSaveToHistory(content, topicKeyFromUrl, typeFromUrl);
            } catch (err) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      setCompletionStatus(prev => ({
        ...prev,
        [topicKey]: { ...prev[topicKey], [materialType]: true }
      }));
      handleSaveToHistory(content, topicKey, materialType);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An unknown error occurred.');
    } finally {
      setIsLoading(false);
      ...
      <main className="container mx-auto px-4 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <aside className="lg:col-span-1">
             <div className="sticky top-28 p-6 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-200/80 dark:border-slate-700/80 rounded-xl shadow-lg space-y-6">
                {/* Topic Input */}
                <div>
                    ...
                    <ProgressTracker status={completionStatus[topicKey] || {}} t={t} language={language} />
                </div>
                ...
                <Chatbot topic={currentTopicText} language={language} t={t} />

                <hr className="border-slate-300 dark:border-slate-600" />
                
                <HistoryViewer
                    history={history[topicKey]?.[materialType] || []}
