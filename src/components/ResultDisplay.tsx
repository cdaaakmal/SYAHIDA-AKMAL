                ...
                {content.map((card, index) => (
                    <div key={index} className="perspective-1000 group" onClick={() => handleFlip(index)}>
                        <div className={`relative w-full h-48 transition-transform duration-500 transform-style-preserve-3d ${flipped[index] ? 'rotate-y-180' : ''}`}>
                            {/* Front of card */}
                            <div className={`absolute w-full h-full backface-hidden flex items-center justify-center p-4 bg-gradient-to-br ${cardColors[index % cardColors.length]} text-white rounded-lg shadow-lg cursor-pointer group-hover:scale-105 transition-transform`}>
